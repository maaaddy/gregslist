 ...
 @impl true
  def mount(_params, _session, socket) do
    ...
    if user do
      ...
      vehicles_items = Enum.filter(vehicles_items, fn item ->
        filter_by_age_restrictions(item, user)
      end)

      {:ok, assign(socket, vehicles_items: vehicles_items, sort_order: "asc", user: user)}
    else
      IO.puts("No current_user found in socket.assigns")
      {:ok, assign(socket, user: nil, vehicles_items: [])}
    end
  end
