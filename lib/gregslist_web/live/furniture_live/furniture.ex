defmodule GregslistWeb.ItemLive.Furniture do
  ...
  def mount(_params, _session, socket) do
    ...
    if user do
      ...
      furniture_items = Enum.filter(furniture_items, fn item ->
        filter_by_age_restrictions(item, user)
      end)

      {:ok, assign(socket, furniture_items: furniture_items, sort_order: "asc", user: user)}
    else
      IO.puts("No current_user found in socket.assigns")
      {:ok, assign(socket, user: nil, furniture_items: [])}
    end
  end
