defmodule GregslistWeb.ItemLive.Technology do
  ...
  def mount(_params, _session, socket) do
    ...
    if user do
      ...
      technology_items = Enum.filter(technology_items, fn item ->
        filter_by_age_restrictions(item, user)
      end)

      {:ok, assign(socket, technology_items: technology_items, sort_order: "asc", user: user)}
    else
      IO.puts("No current_user found in socket.assigns")
      {:ok, assign(socket, user: nil, technology_items: [])}
    end
  end
